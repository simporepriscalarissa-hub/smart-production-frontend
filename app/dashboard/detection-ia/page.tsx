  ...
  useEffect(() => {
    ...
    socket.on('alerte_defaut', (data) => {
      const analyse: AnalyseIA = {
        id: Date.now(),
        conforme: false,
        nb_defauts: data.nb_defauts || 1,
        confiance: data.confiance || 0,
        classe: data.classe || 'defaut',
        ouvrier: data.ouvrier || 'Inconnu',
      ...
      setAlertes(prev => [{
        message: `Défaut détecté — ${data.nb_defauts || 1} défaut(s)`,
        heure: new Date().toLocaleTimeString('fr-FR')
      }, ...prev].slice(0, 5))
    })

    socket.on('nouvelle_production', (data) => {
      if (data.quantiteNonConforme === 0) {
        const analyse: AnalyseIA = {
          id: Date.now(),
          conforme: true,
          nb_defauts: 0,
          confiance: 95,
          classe: 'conforme',
          ouvrier: data.ouvrier ? `${data.ouvrier.prenom} ${data.ouvrier.nom}` : 'Inconnu',
          departement: data.ouvrier?.departement || '—',
          heure: new Date().toLocaleTimeString('fr-FR'),
        }
        setAnalyses(prev => [analyse, ...prev].slice(0, 50))
        setDernierResultat(analyse)
